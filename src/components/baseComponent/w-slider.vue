<template>
    <div class="w-slider">
        <ul class="w-slider-ul">
            <router-link v-for="(item, index) in showList" :key="index" tag="li" :to="{path:item.link}" class="w-slider-li">{{item.value}}</router-link>
        </ul>
    </div>
</template>
...
<script>
    export default {
        ...
        async created () {
            this.getShowList();
            let data = await this.Http.$get('/data/m/publicity/subjectCategory/list')
            console.log('====================================');
            console.log(data);
            console.log('====================================');
        },
